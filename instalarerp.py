...
os.system('yum -y install postgresql13-server postgresql13-docs postgresql13-contrib postgresql13-plperl postgresql13-plpython postgresql13-pltcl postgresql13-test rhdb-utils gcc-objc postgresql13-devel --skip-broken')
os.system('/usr/pgsql-13/bin/postgresql-13-setup initdb')
os.system('systemctl start postgresql-13')
os.system('systemctl enable postgresql-13')
...
archi=open("/var/lib/pgsql/13/data/pg_hba.conf",'w')
...
f = open("/var/lib/pgsql/13/data/postgresql.conf",'r')
chain = f.read()
...
otro = open("/var/lib/pgsql/13/data/postgresql.conf",'w')
otro.write(chain)
otro.close()
s = open("/var/lib/pgsql/13/data/postgresql.conf",'a')
...
os.system('systemctl restart postgresql-13')
...
chain = chain.replace("/var/lib/pgsql/9.1/data/pg_log/","/var/lib/pgsql/13/data/pg_log/")
f.close()
